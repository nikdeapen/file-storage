...
use crate::error::{ErrorInfo, Operation};
use crate::ErrorInfo::Other;
use crate::Path;
...
impl Error {
    ...
    /// Creates a new error.
    pub fn new<P>(path: P, operation: Operation, info: ErrorInfo) -> Self
    where
        P: Into<Path>,
    {
...
impl Display for Error {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "{} error: op={} path={}, info={:?}",
            ff, self.path, self.operation, self.info
        )?;
