...
use crate::op::{FileList, FileListInner};
use crate::Operation::List;
use crate::{Error, FilePath, FolderPath, Path};

pub fn list(folder: &FolderPath) -> Result<FileList, Error> {
        ...
        match std::fs::read_dir(current_folder) {
                ...
                while let Some(next) = read_dir.next() {
                    match next {
                        Ok(dir_entry) => match dir_entry.file_type() {
                            ...
                            Err(error) => return Err(Error::from_error(folder, List, error)),
                        },
                        Err(error) => return Err(Error::from_error(folder, List, error)),
                    }
                }
            ...
            Err(error) => {
                if error.kind() == NotFound && folders.is_empty() && files.is_empty() {
                    ...
                    return Err(Error::from_error(folder, List, error));
                }
            }
        }
    }
