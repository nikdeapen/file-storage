...
///
/// # Base & Extension
/// Paths are divided into a base and an extension. The base is effectively immutable and the
/// extension is a sequence of segments divided by file-separators.
///
/// Unix Example:
/// Path:           /the/path
/// Base:           /
...
/// exhaustive. A path is a folder path if it is equal to its base or ends with a file-separator,
...
mod tests {
    ...
    #[test]
    fn properties() {
        let mut path: StoragePath = unsafe { StoragePath::new_unchecked("/the/path", 1, '/') };
        assert_eq!(path.path(), "/the/path");
        unsafe {
            assert_eq!(path.path_mut_unchecked(), "/the/path");
        }
